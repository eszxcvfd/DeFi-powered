        ...
        if "leads" in existing_tables:
            ...
            if e2e_leads:
                ...
                if "lead_activities" in existing_tables:
                    cursor.execute(
                        f"DELETE FROM lead_activities WHERE lead_id IN ({lead_placeholders})",
                        lead_ids,
                    )
                if "follow_up_reminders" in existing_tables:
                    cursor.execute(
                        f"DELETE FROM follow_up_reminders WHERE lead_id IN ({lead_placeholders})",
                        lead_ids,
                    )
                if "reminder_history" in existing_tables:
                    cursor.execute(
                        f"DELETE FROM reminder_history WHERE lead_id IN ({lead_placeholders})",
                        lead_ids,
                    )
