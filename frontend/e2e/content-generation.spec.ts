...
test("content studio generates and edits a draft", async ({ page }) => {
  const name = `Content E2E ${Date.now()}`;
  ...
  await expect(page.getByTestId("content-context-panel")).toBeVisible();
  ...
  await page.getByTestId("content-edit-draft").first().click();
  await page.getByTestId("content-draft-editor").fill("Edited E2E draft for webinar.\n\nCTA: Learn more");
  await page.getByTestId("content-save-draft").click();
  await expect(page.getByTestId("content-draft").first()).toContainText("Edited E2E draft");
});