...
test("Selenium website discovery progress and reviewable events", async ({ page, request }) => {
  ...
  const campName = `Website Selenium ${Date.now()}`;
  ...
  for (let i = 0; i < 180; i++) {
    ...
    if (ev.ok()) {
      ...
      if (
        rows.some((e) => e.canonical_title.includes("US034 Selenium Summit")) ||
        rows.some((e) => e.canonical_title.includes("Selenium Summit"))
      ) {
        foundTitle = true;
        break;
      }
